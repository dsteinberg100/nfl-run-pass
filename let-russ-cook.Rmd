...
Finally, now that we've built some confidence in the model we're going to build and predict the model on all data since 2016.

```{r}
final_qb_mod <- fit(qb_xgb, post16)
post16$dropback_prob <- predict(final_qb_mod, new_data = post16, type = 'prob')$.pred_1
```
...
```{r}
post16_lmer_dat <- filter(dat, season >= 2016 & 
                   season_type == 'REG' & 
                   down %in% c(1,2,3) &
                   !is.na(qb_dropback) &
                   !is.na(score_differential)) %>%
...
dropbacks <- filter(post16_lmer_dat, qb_dropback == 1)
